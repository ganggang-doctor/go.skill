...
### v1.3 (2026-07-24)
- **审计自动触发扩展**：数据分析管线(scRNA/bulk/空间)、大型中间文件(AnnData>500MB)、pipeline exit 0 均自动触发 4c
- **审计阻断规则**：BLOCKER>0 → 阻断后续步骤 → 修复 → 重新审计 → 放行
- **审计自动预约**：阶段2检测高危标签→workflow_plan自动插入审计锚点
- 修复 scRNA-seq 管线完成后被 4c 遗漏的根因

### v1.2 (2026-07-19)
- **递归编排**：单步发现新需求→动态搜索+自动插入步骤
- **子任务拆分+并行**：单步内部拆多个子步骤并行执行
